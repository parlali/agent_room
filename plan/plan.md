...
- [x] Keep absolute workspace/store/home/tmp paths visible to sandboxed processes for POSIX cwd and tool compatibility, while keeping authentication, session ownership, runtime tokens, and audit IDs backend-only. (Explicit decision after review noted room IDs can appear in host path segments.)
