...
Using Codex as a delegate coder:

- The OpenAI Codex CLI is installed (`codex`). Global config at `~/.codex/config.toml` pins model `gpt-5.5` with `model_reasoning_effort = "xhigh"`.
- Run it non-interactively: `codex exec [OPTIONS] "<prompt>"`. Long tasks should run in the background with output captured to a file. Resume a session with `codex exec resume --last`.
- Give Codex its own git worktree so it never edits this checkout: `git worktree add ../agent_room-codex-<issue> -b <branch>`, run codex with that directory as cwd, and remove the worktree after the PR merges.
- Sandbox flags: `--sandbox read-only` for analysis, `--sandbox workspace-write` for implementation inside the worktree. Codex reads AGENTS.md in its cwd for repo rules.
- Claude orchestrates: write the spec, review every Codex diff against it, run `bun run check`, and iterate with feedback until the diff is correct. Codex output is a proposal, not truth.
