...
function attachmentHandlingInstruction(): string {
    return [
        ...
        'Attached PDFs are provided through native PDF input when the configured provider supports it, otherwise as rendered page images for vision-capable models; use agent_room_read_pdf by default for room-local PDF paths and agent_room_pdf_extract_text only when text extraction is explicitly requested or the PDF read tool reports that native or rendered reading is unavailable.',
        'For DOCX, XLSX, and PPTX create, inspect, and edit workflows, prefer the bundled office-documents skill through agent_room_shell; dedicated office document tools are legacy compatibility paths for export, preview, or structured compatibility gaps.',
