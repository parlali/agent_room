...
export function Deploy() {
                ...
                <div className="mt-12 grid-12">
                    <div className="col-span-12 lg:col-span-5">
                        ...
                        <p className="mt-8 max-w-md text-[15.5px] leading-[1.6] text-[var(--color-ink-dim)]">
                            The whole stack is one Docker Compose file. App, Postgres, internal SearXNG. Generated
                            credentials on first boot. Encrypted secret storage. Local-first by default.
                        </p>
                    ...
                    <div className="col-span-12 mt-16 lg:col-span-7 lg:mt-0">
                        <div className="border border-[var(--color-rule)] bg-[var(--color-night-elev)]">
                            <div className="flex items-center justify-between border-b border-[var(--color-rule)] px-4 py-2.5">
                                <span className="label-mono text-[var(--color-ink)]">STACK · COMPOSE TOPOLOGY</span>
  ...
  └───────────────────────────────────────────────────────────────┘`}</pre>
                        ...
                        <div className="mt-6 grid grid-cols-2 gap-px bg-[var(--color-rule)]">
                            <Spec label="DEFAULT BIND" value="127.0.0.1:3000" hint="local-first by design" />
                            <Spec label="POSTGRES" value="not published" hint="lives on the docker network" />
                            <Spec label="SEARXNG" value="not published" hint="internal, no shared key" />
                            <Spec label="SECRETS" value="encrypted at rest" hint="aes-gcm, per-deployment key" />
                            <Spec label="BOOTSTRAP" value="first-boot" hint="root creds generated, recoverable" />
                            <Spec label="UPDATES" value="git pull · rebuild" hint="migrations run automatically" />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="grid grid-cols-[36px_1fr] gap-4 border-t border-[var(--color-rule)] py-4">
            ...
            <div>
                <div className="flex items-baseline justify-between gap-3">
                    ...
                    <div className="label-mono text-right text-[var(--color-ink-faint)]">{hint}</div>
                </div>
