    ...
    return (
        <div className="flex items-center justify-between gap-6 border-b border-[var(--color-rule)] bg-[var(--color-night)] px-4 py-2 sm:px-6 lg:px-10">
            ...
            <div className="hidden flex-1 overflow-hidden md:block">
                <div className="flex w-[200%] gap-10" style={{ animation: 'ticker 60s linear infinite' }}>
                    {[...items, ...items, ...items, ...items].map((item, i) => (
                        <span key={i} className="label-mono whitespace-nowrap text-[var(--color-ink-dim)]">
                            {item}
            ...
            <div className="flex items-center gap-3">
                ...
                <span className="font-mono text-[11px] tracking-wider text-[var(--color-ink)]">{now}</span>
            </div>
        </div>
    )
}
