...
This is a static SPA. It deliberately does not share runtime, auth, room state, or any private data with the main app. The whole point of keeping it in `apps/site` is to ship a marketing surface without coupling deploys.
...
Output lives at `apps/site/dist`. Drop it on any static host (Cloudflare Pages, Netlify, S3 + CloudFront, GitHub Pages).
