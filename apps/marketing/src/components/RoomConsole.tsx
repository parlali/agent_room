...
const script: Event[] = [
    ...
    { ts: '14:02:14', kind: 'mem', text: 'memory.write - decision - "stick with Q3 vendor"' },
    ...
    return (
        <div className="relative overflow-hidden border border-[var(--color-rule)] bg-[var(--color-night-elev)]">
            <div className="flex items-center justify-between border-b border-[var(--color-rule)] px-4 py-2.5">
                ...
                <span className="label-mono">Online - 12d uptime</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-px bg-[var(--color-rule)]">
                ...
                <Panel
                    label="JOBS"
                    primary="3 active"
                    secondary="next 09:00 PT"
                    hint="daily-report, weekly-recap"
                />
                <Panel
                    label="PROVIDER"
                    primary="codex / oai"
                    secondary="oauth bound"
                    hint="rate limit ok"
                />
            </div>
            ...
            <div className="border-t border-[var(--color-rule)] px-4 py-2.5">
                <div className="flex items-center justify-between">
                    ...
                    <span className="label-mono">Live, idempotent, audited</span>
                </div>
            </div>
            ...
            <div className="h-[220px] overflow-hidden px-4 py-2 font-mono text-[11.5px] leading-[1.65]">
                {visible.map((e, idx) => (
                    ...
                    >
                        <span className="w-[58px] shrink-0 text-[var(--color-ink-faint)]">
                            {e.ts}
                        </span>
                        <span className="w-[58px] shrink-0 text-[10px] text-[var(--color-ink)]">
                            {kindLabel[e.kind]}
                        </span>
...
function Panel({ label, primary, secondary, hint }: PanelProps) {
    return (
        <div className="bg-[var(--color-night-elev)] px-4 py-3">
            ...
            <div className="mt-1.5 text-[20px] font-semibold leading-none text-[var(--color-ink)]">
                {primary}
            </div>
            <div className="mt-1.5 font-mono text-[10.5px] text-[var(--color-ink-dim)]">
                {secondary}
            </div>
            <div className="mt-0.5 font-mono text-[10.5px] text-[var(--color-ink-faint)]">
                {hint}
            </div>
        </div>
    )
